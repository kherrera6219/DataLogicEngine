...
- **Frozen backend missing ONNX Runtime for Chroma collection stats**: `onnxruntime` is now an explicit Python dependency and PyInstaller collects its native runtime files, fixing installed-backend collection-stat errors from Chroma's default ONNX embedding function support.
- **Desktop API-key save still failing after reinstall**: Electron-rendered desktop API requests now declare the `X-Desktop-Auth-*` header names before Chromium CORS/preflight handling, while the Electron main process still replaces the placeholder values with real HMAC signatures. This fixes the remaining Settings -> AI Models Save Model path where `/api/v1/gateway/keys` could fall back to cookie-session CSRF and return `CSRF session token missing`.
