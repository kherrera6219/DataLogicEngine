...
- Restored the complete Dataset Exporter regression suite and Developer Guide
  ...
  partial versions.
- Corrected the Dataset Exporter settings initialization so frontend lint and
...
- Added optional owner-only, default-off runtime usage capture for committed,
  ...
  no in-app training path is enabled.
- Removed the legacy Quad `RefinementWorkflow12Step` compatibility stub so the
  governed `CanonicalRefinementWorkflow` remains the single product authority.
