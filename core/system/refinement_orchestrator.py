...
Used only by the non-production UnitedSystemManager compatibility/reference
stack. The live gateway pipeline uses
``backend.governed_execution.refinement.CanonicalRefinementWorkflow``. This
class remains import-compatible but is not a production assembly entrypoint.
...
WORKFLOW_DISPOSITION = "legacy_system_compatibility_reference"
...
class SystemRefinementOrchestrator:
    ...
    def run_refinement(
        self, initial_artifact: UnifiedArtifactEnvelope, max_iterations: int = 3
    ) -> UnifiedArtifactEnvelope:
        """
        ...
        while iteration < max_iterations:
            iteration += 1
            self.logger.info(
                f"Starting refinement iteration {iteration} for {current_artifact.artifact_id}"
            )
            ...
            if current_artifact.confidence_vector.overall >= self.target_confidence:
                self.logger.info(
                    f"Target confidence reached: {current_artifact.confidence_vector.overall}"
                )
                break

            self.logger.warning(
                f"Confidence {current_artifact.confidence_vector.overall} below threshold. Re-iterating."
            )

        return current_artifact

    def _execute_step(
        self, step_name: str, artifact: UnifiedArtifactEnvelope
    ) -> UnifiedArtifactEnvelope:
        ...
        new_artifact.confidence_vector.overall = min(
            1.0, new_artifact.confidence_vector.overall + 0.05
        )
