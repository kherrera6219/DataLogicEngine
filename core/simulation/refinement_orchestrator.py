...
``core.orchestration.master_workflow``. It is a non-production reference; the
canonical live gateway path is
...
WORKFLOW_DISPOSITION = "legacy_simulation_reference"
...
class SimulationRefinementOrchestrator:
    ...
    def __init__(
        self, config, graph_manager, memory_manager, united_system_manager, ka_loader
    ):
        """
    ...
    def run(
        self,
        qpe_output: Dict[str, Any],
        query_text: str,
        query_topic_uid: str,
        initial_axis_context_scores: Dict[str, float],
        active_location_context: List[str],
        session_id: str,
        pass_num: int,
    ) -> Dict[str, Any]:
        """
        ...
        """
        logging.info(
            f"[{datetime.now()}] RO: Starting 12-step refinement process for session {session_id[:8] if session_id else 'N/A'}, pass {pass_num}"
        )

        # Validate QPE output
        if qpe_output.get("status") != "success":
            error_msg = (
                f"Invalid QPE output: {qpe_output.get('error', 'Unknown error')}"
            )
        ...
        try:
            ...
            refinement_state = self._execute_step_1_analysis_of_qpe_outputs(
                refinement_state
            )
            ...
            refinement_state = self._execute_step_6_logical_flow_enhancement(
                refinement_state
            )
            refinement_state = self._execute_step_7_evidence_strengthening(
                refinement_state
            )
            ...
            refinement_state = self._execute_step_10_answer_quality_evaluation(
                refinement_state
            )
            refinement_state = self._execute_step_11_final_answer_generation(
                refinement_state
            )
            ...
            self.smm.update_memory_entry(
                ...
                updates={
                    "content": {
                        "query_text": query_text,
                        "query_topic_uid": query_topic_uid,
                        "refinement_steps_log": refinement_state[
                            "refinement_steps_log"
                        ],
            ...
            result = {
                ...
                "final_scoring_factors_from_s11": refinement_state.get(
                    "final_scoring_factors", {}
                ),
            }

            logging.info(
                f"[{datetime.now()}] RO: Refinement process complete. Final confidence: {final_confidence:.3f}"
            )
    ...
    def _execute_step_1_analysis_of_qpe_outputs(
        ...
        try:
            ...
            top_confident_personas = persona_confidences[
                :2
            ]  # Top 2 most confident personas

            # Calculate step confidence
            step_confidence = min(
                0.9,
                sum(pc["confidence"] for pc in persona_confidences)
                / len(persona_confidences)
                + 0.1,
            )
            ...
            logging.info(
                f"[{datetime.now()}] RO_S1: Analysis complete. Confidence: {step_confidence:.3f}"
            )
    ...
    def _execute_step_2_initial_synthesis(
        self, state: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        qpe_output = state["qpe_output"]
        personas_output = qpe_output.get("personas_output", {})
        step_1_output = state["refinement_steps_log"].get(
            "S1_Analysis_of_QPE_Outputs", {}
        )
        ...
        try:
            ...
            for persona_type in successful_personas:
                ...
                if persona_output.get("status") == "success":
                    persona_name = persona_output.get("persona_model", {}).get(
                        "name", persona_type
                    )
            ...
            included_top_personas = sum(
                1 for p in top_persona_types if p in successful_personas
            )
            step_confidence = min(
                0.9,
                0.5 + (included_top_personas / max(1, len(top_persona_types))) * 0.4,
            )
            ...
            logging.info(
                f"[{datetime.now()}] RO_S2: Initial synthesis complete. Draft length: {len(draft)}, Confidence: {step_confidence:.3f}"
            )
    ...
    def _execute_step_3_gap_identification(
        self, state: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            missing_key_terms = [
                ...
                if term not in draft_terms and len(term) > 4
            ]

            # Identify gaps based on initial axis context scores
            axis_scores = state["initial_axis_context_scores"]
            high_relevance_axes = [
                axis_id for axis_id, score in axis_scores.items() if score > 0.7
            ]
            ...
            # Calculate step confidence
            gap_severity = (
                len(short_sections) + len(missing_key_terms) + len(axis_coverage_gaps)
            )
            step_confidence = max(
                0.3, min(0.9, 1.0 - (gap_severity / 20))
            ...
            logging.info(
                f"[{datetime.now()}] RO_S3: Gap identification complete. Gaps: {gap_severity}, Confidence: {step_confidence:.3f}"
            )
    ...
    def _execute_step_4_premise_validation(
        self, state: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for line in lines:
                if any(
                    ...
                    for keyword in ["is", "are", "should", "must", "always", "never"]
                ):
            ...
            logging.info(
                f"[{datetime.now()}] RO_S4: Premise validation complete. Validity ratio: {validity_ratio:.3f}, Confidence: {step_confidence:.3f}"
            )
    ...
    def _execute_step_5_context_expansion(
        self, state: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if active_location_context:
                ...
                location_names = [
                    f"Location_{loc_id[-8:]}" for loc_id in active_location_context
                ]
            ...
            logging.info(
                f"[{datetime.now()}] RO_S5: Context expansion complete. Additions: {len(context_additions)}, Confidence: {step_confidence:.3f}"
            )
    ...
    def _execute_step_6_logical_flow_enhancement(
        self, state: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            content_change_ratio = abs(len(enhanced_draft) - len(current_draft)) / max(
                1, len(current_draft)
            )
            ...
            logging.info(
                f"[{datetime.now()}] RO_S6: Logical flow enhancement complete. Structure changes: {structure_changes}, Confidence: {step_confidence:.3f}"
            )
    ...
    def _execute_step_7_evidence_strengthening(
        self, state: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for i, line in enumerate(lines):
                if len(line) > 100 and not line.startswith(
                    "#"
                ):  # Long non-heading lines
                    statements_needing_support.append({"line_index": i, "text": line})
            ...
            strengthening_ratio = len(evidence_additions) / max(
                1, len(statements_needing_support)
            )
            ...
            logging.info(
                f"[{datetime.now()}] RO_S7: Evidence strengthening complete. Additions: {len(evidence_additions)}, Confidence: {step_confidence:.3f}"
            )
    ...
    def _execute_step_8_governance_check(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            governance_flags_count = (
                len(ethics_flags) + len(legal_issues) + len(security_flags)
            )
            governance_severity = (
                "Low"
                if governance_flags_count == 0
                else "Medium"
                if governance_flags_count <= 2
                else "High"
            )
            ...
            if governance_flags_count > 0:
                ...
                if security_flags:
                    disclaimer += (
                        "Security-related information should be handled responsibly. "
                    )
            ...
            governance_confidence_factor = max(
                0.3, 1.0 - (governance_flags_count * 0.1)
            )
            ...
            governance_check_details = {
                "ethics_evaluation": {
                    "flags": ethics_flags,
                    "severity": "Low"
                    if len(ethics_flags) == 0
                    else "Medium"
                    if len(ethics_flags) <= 1
                    else "High",
                },
            ...
            logging.info(
                f"[{datetime.now()}] RO_S8: Governance check complete. Issues: {governance_flags_count}, Severity: {governance_severity}, Confidence: {step_confidence:.3f}"
            )
    ...
    def _execute_step_9_final_synthesis(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            missing_essential_sections = [
                ...
                if not any(s in title for title in section_titles)
            ]
            ...
            for missing_section in missing_essential_sections:
                if missing_section == "introduction":
                    introduction = "\n\n## Introduction\n\n"
                    introduction += (
                        f'This response addresses the query: "{state["query_text"]}". '
                    )
            ...
            if len(section_titles) > 3:
                ...
                for title in [t for t in section_titles if t != "conclusion"]:
                    ...
                    if title != "conclusion":
                        numbered_draft = numbered_draft.replace(
                            f"## {title.title()}",
                            f"## {section_number}. {title.title()}",
                        )
                        section_number += 1
            ...
            synthesis_change_ratio = abs(len(final_draft) - len(current_draft)) / max(
                1, len(current_draft)
            )
            ...
            final_synthesis_details = {
                ...
                "final_section_count": len(section_titles)
                + len(missing_essential_sections),
            }
            ...
            logging.info(
                f"[{datetime.now()}] RO_S9: Final synthesis complete. Change ratio: {synthesis_change_ratio:.3f}, Confidence: {step_confidence:.3f}"
            )
    ...
    def _execute_step_10_answer_quality_evaluation(
        self, state: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            query_terms = set(
                term.lower() for term in state["query_text"].split() if len(term) > 3
            )
            ...
            essential_present = all(
                any(s.lower() in title.lower() for title in section_titles)
                for s in essential_sections
            )
            ...
            correct_order = (
                "introduction" in section_indices
                and "conclusion" in section_indices
                and section_indices["introduction"] < section_indices["conclusion"]
            )

            coherence_score = (0.5 if essential_present else 0.3) + (
            ...
            logging.info(
                f"[{datetime.now()}] RO_S10: Quality evaluation complete. Overall score: {overall_quality_score:.3f}, Confidence: {step_confidence:.3f}"
            )
    ...
    def _execute_step_11_final_answer_generation(
        self, state: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            quality_scores = (
                state["refinement_steps_log"]
                .get("S10_Answer_Quality_Evaluation", {})
                .get("quality_evaluation_details", {})
                .get("quality_scores", {})
            )
            ...
            governance_details = (
                state["refinement_steps_log"]
                .get("S8_Governance_Check", {})
                .get("governance_check_details", {})
            )

            # Extract gaps from Step 3
            gaps_identified = (
                state["refinement_steps_log"]
                .get("S3_Gap_Identification", {})
                .get("gaps_identified_details", {})
            )
            ...
            final_scoring_factors = {
                ...
                "gaps_identified_details": gaps_identified,
                "self_critique_summary": state["refinement_steps_log"]
                .get("S10_Answer_Quality_Evaluation", {})
                .get("quality_evaluation_details", {})
            ...
            key_steps = [
                "S3_Gap_Identification",
                "S8_Governance_Check",
                "S10_Answer_Quality_Evaluation",
            ]
            key_step_confidences = [
                state["step_confidence_scores"].get(step, 0.5) for step in key_steps
            ]
            ...
            quality_score = (
                ...
                .get("S10_Answer_Quality_Evaluation", {})
                .get("quality_evaluation_details", {})
                .get("overall_quality_score", 0.7)
            )

            # Get governance severity
            governance_severity = governance_details.get(
                "overall_governance_severity", "Low"
            )
            governance_penalty = (
                0.0
                if governance_severity == "Low"
                else 0.1
                if governance_severity == "Medium"
                else 0.2
            )
            ...
            final_confidence = min(
                0.95,
                (key_steps_confidence * 0.4 + quality_score * 0.6) - governance_penalty,
            )
            ...
            logging.info(
                f"[{datetime.now()}] RO_S11: Final answer generation complete. Length: {len(final_answer)}, Confidence: {final_confidence:.3f}"
            )
    ...
    def _execute_step_12_meta_learning(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_confidence = sum(step_confidences.values()) / max(
                1, len(step_confidences)
            )
            min_confidence_step = (
                min(step_confidences.items(), key=lambda x: x[1])
                if step_confidences
                else ("none", 1.0)
            )
            max_confidence_step = (
                max(step_confidences.items(), key=lambda x: x[1])
                if step_confidences
                else ("none", 0.0)
            )
            ...
            ineffective_steps = [
                step for step, conf in step_confidences.items() if conf < 0.5
            ]
            ...
            for step, output in state["refinement_steps_log"].items():
                if "error" in output:
                    error_patterns.append({"step": step, "error": output["error"]})
            ...
            logging.info(
                f"[{datetime.now()}] RO_S12: Meta-learning complete. Insights: {len(learning_insights)}, Suggestions: {len(improvement_suggestions)}, Confidence: {step_confidence:.3f}"
            )
    ...
    def _calculate_final_confidence(self, state: Dict[str, Any]) -> float:
        ...
        if "S11_Final_Answer_Generation" in state["refinement_steps_log"]:
            final_confidence = state["refinement_steps_log"][
                "S11_Final_Answer_Generation"
            ].get("final_confidence_score", 0.7)
        else:
    ...
    def _identify_commonalities(
        self, personas_output: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
                ...
                differences.append(
                    {
                        ...
                        "unique_focus": output.get("persona_model", {}).get(
                            "knowledge_focus", "unknown"
                        ),
                    }
                )
