  ...
  return (
    <div className="flex-1 flex flex-col bg-transparent animate-in fade-in duration-700">
      <div className="h-16 border-b border-white/5 fluent-acrylic sticky top-0 z-10 flex items-center justify-between px-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-[10px] text-gray-500 font-mono uppercase tracking-widest">Chat Session Workspace</div>
          </div>
        </div>
        <Button
      ...
      <div className="max-w-[1600px] w-full mx-auto p-8 space-y-8">
        <div className="flex items-center justify-between bg-white/5 p-2 rounded-2xl border border-white/5 backdrop-blur-md shadow-inner">
          <div className="relative w-96">
            ...
            <Input
              ...
              className="pl-9 bg-black/20 border-transparent hover:bg-black/40 focus:bg-black/50 transition-colors h-9 text-sm"
              value={query}
        ...
        {!loading && error && (
          <div className="text-sm text-red-400">Failed to load projects: {error}</div>
        )}
        ...
        {!loading && !error && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  ...
                  <Card className="fluent-acrylic border-white/10 h-full transition-all duration-500 hover:-translate-y-2 relative overflow-hidden group hover:shadow-[0_20px_40px_rgba(0,0,0,0.4)]">
                    ...
                    <CardContent className="p-6">
                      <div className="flex justify-between items-start mb-6">
                        <div className={`h-12 w-12 rounded-2xl ${accent} bg-opacity-20 flex items-center justify-center border border-white/10 shadow-lg group-hover:scale-110 transition-transform duration-500`}>
                      ...
                      <h3 className="font-bold text-xl mb-1 text-gray-100 group-hover:text-blue-400 transition-colors tracking-tight">{title}</h3>
                      <div className="flex items-center gap-2 mb-6">
                        <Badge className="bg-white/5 border-none text-gray-400 text-[9px] font-bold uppercase tracking-widest px-2 py-0">
                          {status}
                        </Badge>
                        <span className="text-[10px] text-gray-600 font-mono">#{session.id.slice(0, 8)}</span>
                      </div>
            ...
            <button
              className="border border-dashed border-white/10 rounded-2xl flex flex-col items-center justify-center p-8 text-gray-500 hover:text-blue-400 hover:border-blue-500/40 hover:bg-blue-500/5 transition-all group h-full min-h-[260px] relative overflow-hidden"
              onClick={() => router.push('/chat')}
            >
              ...
              <div className="text-[10px] text-gray-600 mt-2 font-mono">OPEN CHAT</div>
            </button>
          </div>
        )}
      </div>
