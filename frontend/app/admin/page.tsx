      ...
      <div className="min-h-full bg-[url('/grid-pattern.svg')] bg-[size:40px_40px] bg-fixed">
         ...
         <div className="h-16 border-b border-white/5 fluent-acrylic sticky top-0 z-10 flex items-center justify-between px-8 backdrop-blur-3xl">
            ...
            <div className="flex gap-3">
               <div className="flex items-center gap-2 text-xs text-slate-600 dark:text-gray-400 px-3 py-1 bg-white/70 dark:bg-white/5 rounded-full border border-slate-300/70 dark:border-white/5">
               ...
               {[
                  ...
                  <Card key={i} className="fluent-card hover:-translate-y-1">
                     <CardContent className="p-5 flex items-center justify-between">
                        <div>
                           <div className="text-xs text-slate-500 dark:text-gray-500 uppercase tracking-wider mb-1 font-semibold">{m.label}</div>
                           <div className="text-2xl font-bold font-mono text-slate-900 dark:text-gray-200">{m.value}</div>
                           <div className="text-[10px] text-slate-500 dark:text-gray-500 mt-1">{m.sub}</div>
                        </div>
               ...
               <CardHeader className="border-b border-white/5 pb-4">
                  <div className="flex items-center justify-between">
                     <div>
                        ...
                        <CardDescription className="text-xs text-slate-600 dark:text-gray-500 mt-1">Manage enterprise access keys and permissions.</CardDescription>
                     </div>
                     <div className="flex gap-2">
                        <div className="relative">
                           <Search className="absolute left-2.5 top-2.5 h-3.5 w-3.5 text-slate-500 dark:text-gray-500" />
                           <Input placeholder="Search users..." className="h-9 pl-9 w-64 bg-white/80 dark:bg-black/20 border-slate-300/70 dark:border-white/10 text-sm focus:border-blue-500/50 transition-colors" />
                        </div>
                        <Button variant="outline" size="sm" className="h-9 border-slate-300/70 dark:border-white/10 text-slate-700 dark:text-gray-400 hover:text-slate-900 dark:hover:text-white gap-2 bg-white/70 dark:bg-white/5">
                        ...
                        {[
                           ...
                           <tr key={i} className="hover:bg-white/[0.02] transition-colors group">
                              <td className="p-4 pl-6">
                                 <div className="font-semibold text-slate-900 dark:text-gray-200">{u.name}</div>
                              ...
                              <td className="p-4">
                                 <div className="flex items-center gap-2">
                                    <div className={`h-1.5 w-1.5 rounded-full ${u.status === 'active' ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.4)]' : u.status === 'warning' ? 'bg-yellow-500' : 'bg-slate-400 dark:bg-gray-600'}`}></div>
                                    <span className="capitalize text-xs text-slate-600 dark:text-gray-400">{u.status}</span>
                                 </div>
                              </td>
                              <td className="p-4 font-mono text-xs text-slate-500 dark:text-gray-500">{u.active}</td>
