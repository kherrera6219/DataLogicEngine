...
export default function CloudServicesPage() {
  const providers = PROVIDER_MANIFEST.providers.map((provider) => ({
    name: provider.label,
    model: provider.default_model,
    usage: "Optional external provider for governed AI reasoning requests.",
    data: "Prompt text plus any disclosed retrieved text, attachments, persona context, or tool results selected for the request.",
    residency: "Handled under the provider account, contract, and region settings you configure.",
  }));
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="border-premium bg-background/50">
          ...
          <CardContent className="text-sm text-muted-foreground">
            User profiles, chat history, traces, provider settings, and application data are stored in app-owned services on the Windows workstation. No cloud database or SaaS control plane is required.
          </CardContent>
        </Card>
          ...
          {providers.map(p => (
            <div key={p.name} className="p-6 rounded-2xl bg-muted/30 border border-white/5 hover:border-white/10 transition-all flex flex-col md:flex-row justify-between gap-6">
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground">{p.usage} Supported model: <span className="font-mono">{p.model}</span>.</p>
              </div>
