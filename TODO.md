...
| Phase H / Desktop experience | H-1/H-3 JRE setup and app-owned Java priority are implemented; H-2 installer resource wiring is present; H-4/H-7 network status is exposed through backend and Electron IPC; H-13/H-14 backend health-gated startup splash and three-attempt restart recovery are implemented. Existing IPC also exposes DB/quad/DMRF/DSQP status. Live code review on 2026-05-28 confirmed a chat `LiveTracePanel` and Settings `DatabaseSettings` dashboard exist, but they use REST polling and service-level health rather than the dedicated Phase H IPC feeds and detailed database metrics. | In progress. Remaining H work: Tier 3+ offline queue/replay, DSQP LocalSLM audit mode, dedicated `reasoning-layer-progress` and `ka-execution-feed` IPC channels, trace panel enrichment with active KAs/persona confidence/FROST counts, detailed database metrics dashboard, one-click backup, PyInstaller build evidence, and cold-start profiling. |
...
- [ ] Phase H remaining: implement Tier 3+ offline queue/replay, DSQP LocalSLM audit mode, dedicated live reasoning/KA IPC feeds, trace panel enrichment with active KAs/persona confidence/FROST counts, detailed settings database metrics, one-click backup flow, PyInstaller build evidence, and cold-start profiling.
