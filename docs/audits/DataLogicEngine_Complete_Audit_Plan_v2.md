...
| **A29** | `core/*` | ✅ done | `b1298293` | orphan sweep of all 115 files (rest already audited A9/A11/A13/A6/A21/N1 or scanner-clean). Removed 7 dead: core/algorithms/ (dead parallel KA framework, 3 files), resilience_router, rag_sanitizer (redundant relocations), pov_engine_enterprise (dead "enterprise" L4 wrapper), query_analysis_system. Kept core/data (live ka_registry.json) |
| **A30** | `config/`, `migrations/`, `k8s/` | ✅ done | `5202e980` | **deleted `k8s/`** (base manifests — multi-replica/LoadBalancer, twin of the A25 operator; zero CI/deploy refs; user "choice A"). **Trimmed `config_manager.py`** (A28-deferred): removed 4 dead enterprise services (webhook_server/model_context/core_ukg/dotnet_service) + JWT auth block — zero readers; kept api_gateway(=backend:5000)/frontend/system/database. **Added OLLAMA/local-model block to `.env.template`** (was missing OLLAMA_BASE_URL/PORT/TIMEOUT/LOCAL_SLM_ENDPOINT that gateway+LMA read). **Migrations:** chain linear+single-head; documented in `migrations/README` that the base schema is `create_all` and migrations are deltas (from-scratch `flask db upgrade` is not the bootstrap — intentional). `config.env` KEPT (referenced by runtime_precheck/security_manager). Note: A25's "k8s/ vs deploy/k8s/ dup" was already moot (deploy/k8s/ removed in A25). |
| **A31** | docs | ✅ done | `1215b6ee` | regenerated `GENERATED_STRUCTURE.md`/`FILE_INVENTORY.csv` (1634 files; dropped this run's deletions); `.env.template` — removed dead multi-user SSO (Azure AD/Entra/MS Graph) + Azure Storage + wrong-framework `REACT_APP_*` (kept wired Azure OpenAI key + `NEXT_PUBLIC_API_URL`); renamed stale `test_sanitizer_and_context_aware.py`→`test_sanitizer.py`. `verify_docs_references` 0 errors; deploy/** clean (only binary artifacts match); docs/ already single-mode-clean from A15-B2. `tests/*.md` phase-summaries left as historical snapshots (per B2 precedent). |
| **A32** | scripts | ✅ done | `54c53221` | **retired 12 dead one-off scripts** — superseded scanners (`audit_deep.py`+`audit_duplicates.py`), one-shot doc-patchers (`patch_todo`/`patch_handoff`/`patch_audit_plan_session`/`patch_audit_plan_v2`/`fix_todo_dup`/`dedup_todo_item16`), hardcoded-path route diagnostics (`find_all_routes`/`scan_backend_routes`), one-shot KA codemods (`fix_ka_imports`/`fix_kas`); all unreferenced, hardcoded-path or already-executed. **Guarded `seed_data.py`** (blocks `db.create_all()`+seed when FLASK_ENV=production unless ALLOW_SEED=true). KEPT the reusable parameterized scanners (`find_orphaned_modules.py`/`find_core_backend_inversions.py`). Collection clean (1806); the scanner's other scripts/ "orphans" are legit CLI entry points (import-based scan over-reports for entry-point scripts). |
| **— PHASE 4 COMPLETE (A18–A32) — v2.0 FIRST-PASS AUDIT COMPLETE —** | | ✅ | — | all 4 phases done; 80–90% of major structural problems addressed (per the deliberate-pace plan); a second long-tail pass may follow |

Per-session findings and verdicts are recorded in `REPO_AUDIT_LOG.md`.
Live test baseline (2026-06-24): **1787 passed / 19 skipped / 0 failed** (SQLite; was 1876 — −89 from the ORPH-4 + ORPH-v2 module/test removals).
**ALL PHASES COMPLETE (A1–A32). The v2.0 first-pass audit is done** (2026-06-24) — every planned area audited; ~80–90% of major structural problems addressed per the deliberate-pace plan, with a possible second long-tail pass later.
...
| A3-4 | defense supervisor `user_role` always "user"; HONEYPOT treated as BLOCK | A10 | ✅ resolved 2026-06-22 (`user_role="owner"` already set in C1/C2 sweep; HONEYPOT→BLOCK correct under single-mode — no adversary to decoy in a single-user local app; comment added, HONEYPOT label kept in audit meta) |
| A5-2 | five overlapping pattern-injection defenses (shield/guardrail/supervisor/truthgate/dmrf) — confirm union, consider consolidation | A10 | ✅ resolved 2026-06-22 (verdict: defense-in-depth UNION — different methods at different stages; keep all five; consolidation would shrink coverage; documented in REPO_AUDIT_LOG) |
...
| SC-2 | Encryption: Fernet→AES-256-GCM decision (note: AES-256-GCM landed in Sprint 2 `EncryptionManager`; confirm docs) | A10 / A31 | ✅ verified 2026-06-22 (AES-256-GCM implemented in `encryption_manager.py` w/ legacy Fernet decrypt; active docs already describe it as implemented — no stale target-state docs; remaining Fernet mentions are the separate provider-key path) |
...
| ORPH-4 | `OAuthAccount` ORM model (`models.py:244`) now used only by model-surface tests after ORPH-3 — assess dropping model + `oauth_accounts` table (needs Alembic migration + update `test_models` 65-class pin) | A27 / schema | ✅ resolved 2026-06-24 (user decision DROP; removed `OAuthAccount` from models.py + the `test_models` ORM pin (65→64) + `test_models_extended` import/test; new reversible+idempotent migration `d6e7f8a9b0c1_drop_oauth_accounts_table` chained off head `c5d6e7f8a9b0`; upgrade/downgrade round-trip validated on SQLite; DATABASE_SCHEMA ER diagram updated; 32 model tests pass, ruff clean) |
| ORPH-v2-batch | orphan-scanner v2 (prose-doc exclusion) surfaced 13 candidates — verify per-area, do NOT mass-delete. **schemas/auth_schemas+simulation_schemas ✅ removed in A27.** **i18n ✅ removed in A28.** security/services batch verified + cut 2026-06-24. | A10/A19 | ✅ resolved 2026-06-24 (per-module verify; all zero production importers + not bundled). **Deleted 7** (user decision): security/{honeypot (attacker decoy, dead single-mode), context_aware (Crescendo dup of live defense_supervisor), api_security (HMAC for retired enterprise gateway), security_monitoring (SIEM, multi-user)} + email_service (multi-user auth emails) + export_service (route-level export supersedes) + services/file_upload_service (multimodal_routes hardening supersedes). **Kept 2** (user decision, plausible future compliance value): security/data_classification (PII/GDPR classifier) + security/vulnerability_scanner (CVE/SBOM) — still test-only orphans, reassess if never wired. Trimmed ~7 shared test files (kept their live-code coverage), deleted 1 dedicated; full collection clean (1806), ruff clean, bandit baseline 479→472. NOTE: active_defense + sanitizer surfaced as *new* test-only candidates (out of this batch's scope) → future pass. |
