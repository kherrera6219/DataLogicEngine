...
> **Archived historical snapshot:** Superseded on 2026-07-12 by the compact root
> `HANDOFF.md` and root `PRODUCTION_COMPLETION_PLAN_2026.md`. Do not use the
> session logs or old installer-testing priorities below as current instructions.
...
- **CodeQL source remediation:** alerts #593-#596 and #598-#601 no longer receive raw exception text from search, retention, audit verification, security, or storage services/routes. Detailed exceptions remain server-side; focused disclosure regressions passed 64 tests.
- **Backend CI validation:** the corrected gateway-router limiter fixture removed three setup errors hidden behind the resolver failure. The complete Python 3.11 suite passed 1,699 tests with 18 skips; smoke bootstrap, strict startup precheck, schema parity, repository syntax lint, and documentation references also passed.
- **Remote confirmation complete for `05c34bdd`:** Deploy run `29178879540`, CI/CD run `29178879516`, and Security Scan run `29178879511` all passed. Windows installer packaging and portable-launch smoke passed, both Docker images built, and CodeQL moved #593-#596 plus #598-#601 to `fixed` with zero open alerts.
...
- **Code-scanning remediation complete in source:** all eight medium `py/stack-trace-exposure` findings (#593-#596 and #598-#601) were traced to service result objects and route sinks, sanitized, and covered by regressions. Evidence is recorded in `reports/code_scanning_alerts_2026-07-08.md`.
...
- **Packaging repair:** `npm --prefix frontend run electron:dist` now rebuilds the PyInstaller backend before Electron packaging. The frozen backend explicitly includes `onnxruntime==1.26.0`, compatible `tokenizers==0.22.2`, and their native files after live logs showed Chroma collection-stat dependency errors in the installed backend.
...
- **Installed-app acceptance still required:** reinstall the rebuilt installer, then validate provider save/test behavior for OpenAI and Google plus unsupported legacy-provider status handling.
...
plan: `docs/archive/audits/DataLogicEngine_Auth_Deprecation_Plan.md`):
...
> Full plan + entanglement map: `docs/archive/audits/DataLogicEngine_Auth_Deprecation_Plan.md`.
...
> `docs/archive/audits/DataLogicEngine_Complete_Audit_Plan_v2.md`. Per-session detail
...
> Plan delivered → **`docs/archive/audits/DataLogicEngine_Auth_Deprecation_Plan.md`** (6 phases,
