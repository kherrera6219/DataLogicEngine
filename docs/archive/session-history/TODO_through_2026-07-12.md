...
> **Archived historical snapshot:** Superseded on 2026-07-12 by the open-only
> root `TODO.md` and root `PRODUCTION_COMPLETION_PLAN_2026.md`. Completed and
> older testing queues below are retained for traceability, not current work.
...
Structural audit update: 2026-06-07. Sprints 1, 2, and 3 are complete. Routes audit completed 2026-06-07: 22 route files reviewed across `routes/` and `backend/routes/`; 20 issues identified; RT-1 through RT-18 sprint tasks defined. Complete remaining audit plan produced 2026-06-10: live scan of all 1,049-commit repo identified 32 audit areas across ~36 sessions; the completed v2 plan is retained at `docs/archive/audits/DataLogicEngine_Complete_Audit_Plan_v2.md` and the superseded v1 snapshot was deleted during the 2026-07-06 docs cleanup. Sprint 1 eliminated duplicate class names, module name collisions, and misplaced files. Sprint 2 resolved all core→backend import inversions (`find_core_backend_inversions.py` reports 0 lines; `# inversion:ok` policy documented in `REPO_AUDIT_LOG.md`). Sprint 3 replaced all 5 stub `_check_*` compliance methods with real SOC 2 Type 2 runtime checks (SC-1 through SC-5) plus 25 unit tests. Full suite: 1855 passed / 21 skipped / 0 failures.
...
16. [x] REPO-AUDIT-COMPLETE-PLAN-V2: complete remaining audit plan v2.0 — all 4 new items investigated from live code reads + full conversation history review.
    ...
    - Historical plan (completed and superseded): `docs/archive/audits/DataLogicEngine_Complete_Audit_Plan_v2.md`
...
35. [~] AUDIT-A15: Phase 3 — `frontend/app/` pages audit + deferred auth removals. **IN PROGRESS.**
    ...
    - **Deferred auth removal — Phase D + E-1/E-2a/E-2b DONE 2026-06-19** (historical plan: `docs/archive/audits/DataLogicEngine_Auth_Deprecation_Plan.md`):
...
- ✅ **Dual-engine (SQLite+Postgres) parity**: `validate_schema_parity.py` gate passes (0 errors/warnings — schema
  ...
  (Postgres wasn't running this session). **Remaining A18:** resilience-test fault injection — then A19.
    ...
    - Historical auth deprecation plan (completed and superseded): `docs/archive/audits/DataLogicEngine_Auth_Deprecation_Plan.md`. The old "Phases D+E+F remain" wording is obsolete; the plan records all phases complete.
...
30. [x] AUDIT-A10: Phase 2 — `backend/security/` audit + **auth deprecation BANKED at A+B+C-partial**.
    ...
    - Historical plan (completed and superseded): `docs/archive/audits/DataLogicEngine_Auth_Deprecation_Plan.md`. The partial status below is a point-in-time record, not current work.
