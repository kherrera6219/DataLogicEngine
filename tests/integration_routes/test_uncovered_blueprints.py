...
import backend.pillar_api as pillar_api_module
import backend.security_api as security_api_module
import backend.security_scan_api as security_scan_api_module
...
@pytest.fixture
def security_scan_app():
    app = Flask(__name__)
    app.config["TESTING"] = True
    app.register_blueprint(security_scan_api_module.scan_bp)
    return app


@pytest.fixture
def security_scan_client(security_scan_app):
...
def test_security_scan_api_endpoints(security_scan_client, tmp_path, monkeypatch):
    ...
    with patch("backend.auth.api_decorators.check_api_auth", return_value=(True, admin)), patch.object(
        ...
        resp = security_scan_client.post("/api/security/scan")
        assert resp.status_code == 200
        assert resp.json["scan_id"] == "scan-xyz"

        resp = security_scan_client.get("/api/security/scan/scan-xyz")
        assert resp.status_code == 200
        assert resp.json["scan_id"] == "scan-xyz"

        scan_file = tmp_path / "logs" / "security" / "scan_scan-old.json"
        scan_file.write_text('{"scan_id":"scan-old","timestamp":"2024-01-01T00:00:01","vulnerabilities":[],"warnings":[]}', encoding="utf-8")
        resp = security_scan_client.get("/api/security/scan/scan-old")
        assert resp.status_code == 200
        assert resp.json["scan_id"] == "scan-old"

        resp = security_scan_client.get("/api/security/scan/recent")
        assert resp.status_code == 200
        assert resp.json["status"] == "success"

        resp = security_scan_client.get("/api/security/scan/verify/SEC-001")
        assert resp.status_code == 200
        assert resp.json["verification"]["category"] == "security"

        resp = security_scan_client.get("/api/security/scan/verify/UNKNOWN-999")
        assert resp.status_code == 400
...
def test_security_scan_api_error_paths(security_scan_client):
    ...
    with patch("backend.auth.api_decorators.check_api_auth", return_value=(True, admin)), patch.object(
        ...
        resp = security_scan_client.get("/api/security/scan/missing")
...
def pillar_app():
    app = Flask(__name__)
    app.config["TESTING"] = True
    app.register_blueprint(pillar_api_module.pillar_api, url_prefix="/pillar")
    return app


@pytest.fixture
def pillar_client(pillar_app):
    return pillar_app.test_client()


def test_pillar_api_success_paths(pillar_app, pillar_client):
    manager = MagicMock()
    manager.get_all_pillar_levels.return_value = {"status": "success", "pillars": [{"id": "p1"}]}
    manager.get_pillar_level.return_value = {"status": "success", "pillar": {"id": "p1", "sublevels": []}}
    manager.create_pillar_level.return_value = {"status": "success", "pillar": {"id": "p2"}}
    manager.add_sublevel.return_value = {"status": "success", "sublevel": {"id": "s1"}}
    manager.get_sublevel.return_value = {"status": "success", "sublevel": {"id": "s1"}}
    manager.get_dynamic_mappings.return_value = {"status": "success", "mappings": [{"id": "m1"}]}
    manager.create_dynamic_mapping.return_value = {"status": "success", "mapping": {"id": "m2"}}
    manager.dynamic_sublevel_expansion.return_value = {"status": "success", "expansion": {"new": 1}}
    manager.analyze_text_for_pillar_context.return_value = {"status": "success", "context": {"match": True}}
    manager.export_pillar_levels_to_yaml.return_value = {
        "status": "success",
        "message": "ok",
        "file_path": "pillar.yaml",
    }
    pillar_app.config["KNOWLEDGE_MANAGER"] = manager

    resp = pillar_client.get("/pillar/")
    assert resp.status_code == 200
    assert resp.json["success"] is True

    resp = pillar_client.get("/pillar/p1")
    assert resp.status_code == 200

    resp = pillar_client.post("/pillar/", json={"name": "P2"})
    assert resp.status_code == 201

    resp = pillar_client.get("/pillar/p1/sublevels")
    assert resp.status_code == 200

    resp = pillar_client.post("/pillar/p1/sublevels", json={"sublevel_id": "s1", "name": "Sub 1"})
    assert resp.status_code == 201

    resp = pillar_client.get("/pillar/p1/sublevels/s1")
    assert resp.status_code == 200

    resp = pillar_client.get("/pillar/mappings")
    assert resp.status_code == 200

    resp = pillar_client.post(
        "/pillar/mappings",
        json={
            "source_pillar_id": "p1",
            "source_sublevel_id": "s1",
            "target_pillar_id": "p2",
            "target_sublevel_id": "s2",
        },
    )
    assert resp.status_code == 201

    resp = pillar_client.post("/pillar/p1/expand", json={"context_text": "expand"})
    assert resp.status_code == 200

    resp = pillar_client.post("/pillar/analyze-text", json={"text": "pillar context"})
    assert resp.status_code == 200

    resp = pillar_client.post("/pillar/export", json={"file_path": "pillar.yaml"})
    assert resp.status_code == 200


def test_pillar_api_error_paths(pillar_app, pillar_client):
    pillar_app.config["KNOWLEDGE_MANAGER"] = None
    resp = pillar_client.get("/pillar/")
    assert resp.status_code == 500

    manager = MagicMock()
    manager.get_pillar_level.return_value = {"status": "error", "message": "not found"}
    manager.get_dynamic_mappings.return_value = {"status": "error", "message": "bad filters"}
    manager.create_dynamic_mapping.return_value = {"status": "error", "message": "bad mapping"}
    manager.dynamic_sublevel_expansion.return_value = {"status": "error", "message": "cannot expand"}
    manager.analyze_text_for_pillar_context.return_value = {"status": "error", "message": "bad text"}
    manager.export_pillar_levels_to_yaml.return_value = {"status": "error", "message": "cannot export"}
    pillar_app.config["KNOWLEDGE_MANAGER"] = manager

    resp = pillar_client.get("/pillar/missing")
    assert resp.status_code == 404

    resp = pillar_client.post("/pillar/p1/sublevels", json={"name": "missing id"})
    assert resp.status_code == 400

    resp = pillar_client.get("/pillar/mappings")
    assert resp.status_code == 400

    resp = pillar_client.post("/pillar/mappings", json={})
    assert resp.status_code == 400

    resp = pillar_client.post("/pillar/p1/expand", json={"context_text": "x"})
    assert resp.status_code == 400

    resp = pillar_client.post("/pillar/analyze-text", json={})
    assert resp.status_code == 400
