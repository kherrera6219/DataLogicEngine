...
def sample_trace() -> dict:
    return {
        ...
        "stages": [
            {
                "stage": "L1_Context_Init",
                "status": "completed",
                "details": "Mapped to 17 axes.",
            },
            {
                "stage": "L3_Quad_Personas",
                "status": "completed",
                "details": "Executed P8, P9, P10, P11.",
            },
        ],
        "personas": [
            {"persona_id": "P8_Knowledge", "summary": "Verified domain correctness."},
            {
                "persona_id": "P10_Regulatory",
                "summary": "Enforced California health privacy regulations.",
            },
        ],
    }
...
def test_path_traversal_prevention(tmp_path: Path):
    ...
    with pytest.raises(SecurityError, match="outside"):
        PrivacyRedactor.validate_safe_path(
            tmp_path.parent / "outside.parquet", base_dir=tmp_path
        )
...
def test_dpo_export_requires_real_rejected_candidate(
    tmp_path: Path, sample_trace: dict
):
...
def test_dataset_routes_require_owner_authentication(dataset_app: Flask):
    ...
    assert client.get("/api/v1/dataset/capture-settings").status_code == 401
    assert (
        client.put(
...
def test_dataset_stats_uses_release_candidate_query(dataset_app: Flask):
    ...
    with (
        ...
        patch(
            "backend.routes.dataset_routes.capture_stats",
            return_value={
                "staged_capture_rows": 3,
                "last_capture_at": "2026-08-21T00:00:00+00:00",
            },
        ),
        patch(
    ...
    assert payload["capture_default"] is False
    assert payload["staged_capture_rows"] == 3
    assert payload["last_capture_at"] == "2026-08-21T00:00:00+00:00"


def test_capture_settings_default_off(dataset_app: Flask):
    payload = {
        "enabled": False,
        "default": False,
        "policy": "export-only",
...
def test_capture_settings_toggle_is_owner_audited(dataset_app: Flask):
    ...
    setter = MagicMock(
        return_value={"flag_key": "training_data_capture_enabled", "value": True}
    )
    with (
        patch(
            "backend.auth.api_decorators.check_desktop_request_auth",
        ...
        patch(
            "backend.routes.dataset_routes.get_authenticated_principal",
            return_value=principal,
        ),
        patch(
            "backend.routes.dataset_routes.set_training_data_capture_enabled",
            setter,
        ),
        patch(
            "backend.routes.dataset_routes.get_capture_settings_payload",
        ...
        response = dataset_app.test_client().put(
            "/api/v1/dataset/capture-settings",
    ...
    setter.assert_called_once_with(
        ...
        reason="owner_enable_runtime_capture",
    )


def test_capture_settings_reject_non_boolean(dataset_app: Flask):
    with patch(
        "backend.auth.api_decorators.check_desktop_request_auth",
        return_value=(True, SimpleNamespace(id=7, username="owner")),
    ):
...
def test_training_admission_route_uses_app_owned_roots_and_principal(
    ...
    assert service.submit_training_admission.call_args.kwargs["principal_id"] == "7"
    assert (
        service.submit_training_admission.call_args.kwargs["idempotency_key"]
