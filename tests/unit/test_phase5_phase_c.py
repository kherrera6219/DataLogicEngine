...
from backend.knowledge_algorithms.ka_38_consensus_engine import (
    KA038ConsensusEngine,
    KA038Input,
    PersonaClaim,
)
    ...
    synthesized = enhancer._synthesize_responses(
        {
            "knowledge_expert": {
                ...
                "response": "Knowledge view",
                "confidence": 0.8,
            },
            "sector_expert": {
                "role": "Sector Expert",
                "response": "Sector view",
                "confidence": 0.8,
            },
            "regulatory_expert": {
                "role": "Regulatory Expert",
                "response": "Regulatory view",
                "confidence": 0.8,
            },
            "compliance_expert": {
                "role": "Compliance Expert",
                "response": "Compliance view",
                "confidence": 0.8,
            },
        },
        {
...
async def test_refinement_orchestrator_requires_explicit_validator_convergence():
    ...
    result = await orchestrator.refine(
        {"content": "draft", "confidence": 0.4}, {"session_id": "s1"}
    )
...
def test_dynamic_weights_and_ka038_are_json_serializable():
    ...
    weights = weights_engine.compute_all_weights(
        {"regulatory_urgency": 1.0, 12: "axis-key"}
    )
    json.dumps(weights)
    ...
    assert result["substantive_consensus_claimed"] is False
