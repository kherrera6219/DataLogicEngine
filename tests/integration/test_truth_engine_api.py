...
def test_truth_engine_session_flow(client):
    ...
    response = client.post('/api/truth/core/session', 
...
def test_truth_memory_stats(client):
    """Test Truth Memory stats endpoint."""
    response = client.get('/api/truth/memory/stats')
    assert response.status_code == 200

def test_truth_link_stats(client):
    """Test Truth Link stats endpoint."""
    response = client.get('/api/truth/link/stats')
    assert response.status_code == 200

def test_truth_gate_stats(client):
    """Test Truth Gate stats endpoint."""
    response = client.get('/api/truth/gate/stats')
    assert response.status_code == 200

def test_truth_core_tiers(client):
    """Test Truth Core tiers endpoint."""
    response = client.get('/api/truth/core/tiers')
    assert response.status_code == 200
    data = response.get_json()
...
def test_compliance_report(client):
    ...
    response = client.get('/api/truth/compliance/report?tenant_id=tenant_abc')
