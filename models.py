...
class KAProductRun(db.Model):
    """Durable principal-owned plan and execution record for the KA product API."""

    __tablename__ = "ka_product_runs"
    __table_args__ = (
        db.UniqueConstraint(
            "user_id",
            "principal_key",
        ...
        Index("ix_ka_product_run_state_created", "status", "created_at"),
        Index("ix_ka_product_run_user_created", "user_id", "created_at"),
        Index("ix_ka_product_run_request_id", "request_id"),
        {"extend_existing": True},
    )
    ...
    idempotency_key = db.Column(db.String(128), nullable=False)
    request_sha256 = db.Column(db.String(64), nullable=False)
    canonical_id = db.Column(db.String(50), nullable=False)
    manifest_version = db.Column(db.String(80), nullable=False)
    ...
    mode = db.Column(db.String(24), nullable=False, default="production")
    risk_tier = db.Column(db.String(24), nullable=False, default="read_only")
    confirmation_required = db.Column(db.Boolean, nullable=False, default=False)
    confirmation_digest = db.Column(db.String(64), nullable=True)
    confirmation_expires_at = db.Column(db.DateTime, nullable=True)
    confirmed_at = db.Column(db.DateTime, nullable=True)
    plan_payload = db.Column(JSON, nullable=False)
    request_encryption = db.Column(db.String(32), nullable=False)
    request_ciphertext = db.Column(db.Text, nullable=False)
    result_encryption = db.Column(db.String(32), nullable=True)
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Return only content-free metadata safe for the owning principal."""
        run_id = str(self.id)
        return {
            ...
            "completed_at": self.completed_at.isoformat() if self.completed_at else None,
            "expires_at": self.expires_at.isoformat() if self.expires_at else None,
            "status_url": f"/api/v1/ka/runs/{run_id}",
            "execute_url": f"/api/v1/ka/runs/{run_id}/execute",
            "cancel_url": f"/api/v1/ka/runs/{run_id}/cancel",
            "result_url": f"/api/v1/ka/runs/{run_id}/result",
            "trace_url": f"/api/v1/ka/runs/{run_id}/trace",
            "artifacts_url": f"/api/v1/ka/runs/{run_id}/artifacts",
            "effects_url": f"/api/v1/ka/runs/{run_id}/effects",
        }
