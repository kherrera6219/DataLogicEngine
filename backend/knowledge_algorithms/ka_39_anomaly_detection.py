...
class KA039AnomalyDetection(KnowledgeAlgorithm):
    ...
    def _run_logic(self, input_data: KA039Input) -> Dict[str, Any]:
        ...
        anomalies = self._detect_numeric_anomalies(numeric, input_data.method, input_data.threshold)
    ...
    @staticmethod
    def _detect_numeric_anomalies(values: List[float], method: str, threshold: float) -> List[Dict[str, Any]]:
        if len(values) < 3:
            return []
        ...
        if stdev == 0:
            return []
        return [
            {"index": index, "value": value, "score": zscore, "reason": "zscore_threshold"}
            for index, value in enumerate(values)
