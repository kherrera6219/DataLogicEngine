...
class KA048EntityExtraction(KnowledgeAlgorithm):
    ...
    def _run_logic(self, input_data: KA048Input) -> Dict[str, Any]:
        ...
        return {
            ...
            "deterministic": True,
            "limitations": (
                "Entity types are regex match classes, not model-inferred identity "
                "or disambiguation results."
            ),
        }
    ...
    def _extract_entities(text: str) -> List[Dict[str, Any]]:
        patterns = [
            ...
            (
                "REGULATION",
                re.compile(
                    r"\b(?:FAR|DFARS|HIPAA|SOX|GDPR|NIST|ISO)\b(?:[-\s]?\d+(?:\.\d+)*)?",
                    re.IGNORECASE,
                ),
            ),
            (
                "ORG",
                re.compile(
                    r"\b[A-Z][A-Za-z&.-]+(?:\s+[A-Z][A-Za-z&.-]+)*(?:\s+(?:Inc|LLC|Corp|Corporation|Agency|Department|Office|Authority))\b"
                ),
            ),
