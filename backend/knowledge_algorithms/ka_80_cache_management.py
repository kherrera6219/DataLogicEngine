...
class KA080CacheManagement(KnowledgeAlgorithm):
    ...
    def _run_logic(self, input_data: KA080CacheInput) -> Dict[str, Any]:
        ...
        self.log_execution_step("Managing Cache Objects", {"op": operation, "key": cache_key})

        entries = self._entries(state)
        plan = self._operation_plan(operation, cache_key, input_data.value, input_data.ttl_seconds, entries)
    ...
    def _operation_plan(self, operation: str, key: str, value: Any, ttl_seconds: Any, entries: Dict[str, Any]) -> Dict[str, Any]:
        ttl = self._safe_int(ttl_seconds, self.config.get("default_ttl_seconds", 3600))
        if operation == "get":
            return {"operation": operation, "key": key, "result": "HIT" if key in entries else "MISS", "ttl_seconds": ttl}
        if operation == "set":
            return {"operation": operation, "key": key, "result": "WRITE_PLANNED", "value_size_bytes": len(str(value).encode()), "ttl_seconds": ttl}
        if operation in {"evict", "delete"}:
            return {"operation": operation, "key": key, "result": "EVICT_PLANNED" if key in entries or key == "*" else "NOOP_MISSING_KEY"}
        if operation == "clear":
            return {"operation": operation, "key": key, "result": "CLEAR_PLANNED", "affected_entries": len(entries)}
        return {"operation": operation, "key": key, "result": "STATS_ONLY", "ttl_seconds": ttl}

    def _stats(self, entries: Dict[str, Any]) -> Dict[str, Any]:
        memory_bytes = sum(len(str(key).encode()) + len(str(value).encode()) for key, value in entries.items())
        max_memory = self._safe_int(self.config.get("max_memory_mb", 2048), 2048) * 1024 * 1024
        stale = sum(1 for value in entries.values() if isinstance(value, dict) and value.get("stale"))
        hits = sum(int(value.get("hits", 0)) for value in entries.values() if isinstance(value, dict))
        misses = sum(int(value.get("misses", 0)) for value in entries.values() if isinstance(value, dict))
        total = hits + misses
        return {
            ...
            "memory_pressure": round(memory_bytes / max_memory, 4) if max_memory else 0.0,
        }

    @staticmethod
    def _consistency_status(entries: Dict[str, Any]) -> str:
        if any(isinstance(value, dict) and value.get("stale") for value in entries.values()):
