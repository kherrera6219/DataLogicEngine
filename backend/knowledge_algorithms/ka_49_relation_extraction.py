...
class KA049RelationExtraction(KnowledgeAlgorithm):
    ...
    RELATION_PATTERNS = [
        ("owns", re.compile(r"(?P<s>[\w\s&.-]+?)\s+(?:owns|owned by|controls)\s+(?P<o>[\w\s&.-]+)", re.I)),
        ("works_for", re.compile(r"(?P<s>[\w\s&.-]+?)\s+(?:works for|employed by|reports to)\s+(?P<o>[\w\s&.-]+)", re.I)),
        ("located_in", re.compile(r"(?P<s>[\w\s&.-]+?)\s+(?:located in|based in|operates in)\s+(?P<o>[\w\s&.-]+)", re.I)),
        ("depends_on", re.compile(r"(?P<s>[\w\s&.-]+?)\s+(?:depends on|requires|uses)\s+(?P<o>[\w\s&.-]+)", re.I)),
        ("complies_with", re.compile(r"(?P<s>[\w\s&.-]+?)\s+(?:complies with|must follow|is governed by)\s+(?P<o>[\w\s&.-]+)", re.I)),
    ]
    ...
    def _run_logic(self, input_data: KA049Input) -> Dict[str, Any]:
        ...
        self.log_execution_step("Extracting Relations", {"entity_count": len(entities), "text_len": len(text)})
        ...
        for predicate, pattern in cls.RELATION_PATTERNS:
            for match in pattern.finditer(text):
                ...
                if subject and obj and subject != obj:
                    relations.append({"subject": subject, "predicate": predicate, "object": obj, "confidence": 0.82, "source": "pattern"})
        return relations

    @classmethod
    def _proximity_relations(cls, text: str, entities: List[str]) -> List[Dict[str, Any]]:
        relations = []
        lowered = text.lower()
    ...
    def _entity_text(entity: Any) -> str:
        if isinstance(entity, dict):
            return str(entity.get("text") or entity.get("name") or entity.get("id") or entity)
        return str(entity)
