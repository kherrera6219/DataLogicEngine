...
class KA041AbductiveReasoning(KnowledgeAlgorithm):
    ...
    def _run_logic(self, input_data: KA041Input) -> Dict[str, Any]:
        ...
        candidates = input_data.explanations or self._explanations_from_rules(input_data.rules, observation)
        if not candidates:
            candidates = [{"hypothesis": token, "rationale": "Observed salient term"} for token in sorted(self._tokens(observation))[:4]]

        ranked = [self._score_explanation(item, observation, input_data.evidence, input_data.rules) for item in candidates]
        ranked.sort(key=lambda item: (-item["likelihood"], item["hypothesis"]))
        best = ranked[0] if ranked else {"hypothesis": "Unknown", "likelihood": 0.0, "rationale": "No explanations supplied."}
    ...
    @classmethod
    def _score_explanation(cls, item: Any, observation: str, evidence: List[Any], rules: List[Any]) -> Dict[str, Any]:
        ...
        prior = float(explanation.get("prior", explanation.get("likelihood", 0.45)) or 0.45)
        likelihood = round(min(0.95, max(0.05, prior * 0.25 + overlap * 0.35 + rule_support * 0.25 + evidence_mentions * 0.15)), 3)
        return {
            "hypothesis": hypothesis,
            "likelihood": likelihood,
            "rationale": explanation.get("rationale") or "Ranked by local evidence fit.",
            "signals": {
                "prior": round(prior, 3),
    ...
    @classmethod
    def _explanations_from_rules(cls, rules: List[Any], observation: str) -> List[Dict[str, Any]]:
        explanations = []
        obs_terms = cls._tokens(observation)
        for rule in rules:
            text = cls._text(rule)
            ...
            explanations.append({"hypothesis": hypothesis, "rationale": f"Derived from rule: {text}", "prior": 0.55})
        return explanations
    ...
    def _rule_support(cls, hypothesis: str, rules: List[Any]) -> float:
        ...
        supporting = sum(1 for rule in rules if hyp_terms & cls._tokens(cls._text(rule)))
    ...
    def _normalize_explanation(item: Any) -> Dict[str, Any]:
        if isinstance(item, dict):
            return {
                **item,
                "hypothesis": str(item.get("hypothesis") or item.get("name") or item.get("cause") or item),
            }
