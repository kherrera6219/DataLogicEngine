...
class OPAPolicyEvaluator:
    ...
    def __init__(
        self, *, binary_path: str | None = None, policy_path: str | Path | None = None
    ):
        self.binary_path = (
            binary_path
            or os.environ.get("DLE_OPA_BINARY")
            or os.environ.get("OPA_BINARY")
        )
        self.policy_path = Path(
            policy_path
            or os.environ.get("DLE_TRUTHGATE_POLICY")
            or "policies/truthgate.rego"
        )

    def evaluate(self, input_data: dict[str, Any]) -> dict[str, Any]:
        if (
            self.binary_path
            and Path(self.binary_path).exists()
    ...
    @staticmethod
    def _evaluation_error() -> dict[str, Any]:
        return {
            "available": False,
            "backend": "error",
            "allow": False,
    ...
    def _fallback(input_data: dict[str, Any]) -> dict[str, Any]:
        ...
        violations = []
        if (
            risk_domain in {"healthcare", "finance", "legal", "safety"}
        ...
        if input_data.get("axis_17_requires_human") and not input_data.get(
            "human_reviewed"
        ):
