...
# Test/compatibility injection point only. The legacy refinement module is
# imported lazily by the private historical workflow and is not part of the
# governed product assembly.
RefinementOrchestrator: Any = None
...
class TruthCoreEngine:
    ...
    def __init__(
        ...
        try:
            ...
            self.emergence_gate = EmergenceDetectionController(
                ka_controller=ka_controller
            )
        except ImportError:
        ...
        self.persona_enhancer = PersonaEnhancer(
            quad_persona_engine=self.ka_controller
            if hasattr(self.ka_controller, "process_with_persona")
            else None
        )
        try:
        ...
        self.refinement_orchestrator = (
            RefinementOrchestrator(ka_controller=self.ka_controller)
            if RefinementOrchestrator is not None
            else None
        )
        try:
                ...
                executed.append(
                    {
                        ...
                        "duration_ms": max(
                            0, int((time.perf_counter() - start) * 1000)
                        ),
                    }
                )
                break
        ...
        transitions.append(
            {
                ...
                "reason": "all_selected_steps_completed"
                if ok
                else "selected_step_failed",
            }
        )
        failure = None
        if not ok:
            ...
            failure = {
                ...
                "message": failed_step.get("error")
                if failed_step
                else "No TruthCore step executed",
            }
        return {
    ...
    async def create_session(
        ...
        logger.info(
            f"Created TruthCore session {session_id} with tier={tier}, profile={routing_profile}"
        )
        ...
        if session_id not in self.active_sessions:
            if self.db_session:
                try:
                    ...
                    db_sess = (
                        self.db_session.query(TruthSession)
        ...
        try:
            ...
            governed = await LLMGateway(db_session=self.db_session).execute(
                GovernedRequest(
                    ...
                    principal_id=str(session.get("user_id"))
            ...
            profiles = ((result.get("metadata") or {}).get("dsqp") or {}).get(
                "profiles"
            ) or {}
    ...
    def _refresh_graph_context(
        self, query: str, working_context: dict[str, Any]
    ) -> None:
        ...
        try:
            ...
            for axis_number, axis_text in axis_queries:
                memory_nodes.extend(
                    memory_graph.coordinate_nodes(
                        axis_number=axis_number, text=axis_text, limit=5
                    )
                )
                neo4j_nodes.extend(
                    graph_store.find_coordinate_nodes(
                        axis_number=axis_number, text=axis_text, limit=5
                    )
                )

            working_context["memory_graph_nodes"] = self._dedupe_graph_nodes(
                memory_nodes
            )
    ...
    @staticmethod
    def _axis_queries_from_coordinate_vector(
        coordinate_vector: dict[str, Any],
    ) -> list[tuple[int, str]]:
        ...
        axes = (
            coordinate_vector.get("axes")
            if isinstance(coordinate_vector.get("axes"), dict)
            else coordinate_vector
        )
        ...
        for raw_axis in candidates:
            ...
            value = axes.get(
                ...
                axes.get(
                    str(raw_axis),
                    axes.get(f"axis_{axis_number}", axes.get(f"A{axis_number}")),
                ),
            )
    ...
    def _active_persona_axes(working_context: dict[str, Any]) -> list[int]:
        ...
        coordinate_vector = working_context.get("coordinate_vector") or {}
        active_axes = (
            coordinate_vector.get("active_axes")
    ...
    @staticmethod
    def _coordinate_path_for_axis(
        axis_number: int, working_context: dict[str, Any]
    ) -> str:
        coordinate_vector = working_context.get("coordinate_vector") or {}
        axes = (
            coordinate_vector.get("axes")
            if isinstance(coordinate_vector, dict)
            and isinstance(coordinate_vector.get("axes"), dict)
            else coordinate_vector
        )
        raw_value = (
            axes.get(axis_number)
            or axes.get(str(axis_number))
            or axes.get(f"axis_{axis_number}")
            or axes.get(f"A{axis_number}")
        )
    ...
    def _construct_l5_personas(self, working_context: dict[str, Any]) -> dict[str, Any]:
        ...
        persona_context = {
            **working_context,
            "dsqp_mode": working_context.get("dsqp_mode", True),
        }
        for axis_number in self._active_persona_axes(working_context):
            coordinate_path = self._coordinate_path_for_axis(
                axis_number, working_context
            )
            try:
                profile = self.persona_construction.construct_persona(
                    axis_number, coordinate_path, persona_context
                )
                profiles[str(axis_number)] = profile.to_dict()
            except Exception as exc:
                logger.debug(
                    "Persona construction skipped for axis %s: %s", axis_number, exc
                )
        return profiles
    ...
    async def _execute_workflow(
        self, query: str, context: dict[str, Any], steps: list[str], tier: str
    ) -> dict[str, Any]:
        ...
        for step in steps:
            ...
            if self.memory_service:
                try:
                    ...
                    working_context["memory_context"][layer] = [
                        self._memory_vertex_payload(vertex) for vertex in recalled
                    ]
                except Exception as exc:
                    logger.debug(
                        "TruthCore memory recall skipped for %s: %s", layer, exc
                    )
            ...
            if self.memory_service and step_result.get("status") == "completed":
                try:
                    ...
                    logger.debug(
                        "TruthCore memory consolidation skipped for %s: %s", layer, exc
                    )
            ...
            if step_result["status"] == "completed":
                ...
                if step == "intent_parsing":
                    ...
                    if constructed_profiles:
                        ...
                        working_context["constructed_persona_profiles"] = (
                        ...
                        personas_used.update(
                            profile.get("persona_type", str(axis))
                            for axis, profile in constructed_profiles.items()
                        )
                    ...
                    sufficiency = self.sufficiency_tool.evaluate(
                        ...
                        working_context["persona_results"],
                        {
                            "domain": working_context.get("risk_domain", "standard"),
                            "tags": working_context.get("tags", []),
                        },
                    )
                    logger.info(
                        f"SUFFICIENCY DECISION: {sufficiency['mode']} (Reasons: {sufficiency['reasons']})"
                    )
                    ...
                    if sufficiency["mode"] == "expanded_committee":
                        logger.info(
                            f"SUFFICIENCY TRIGGER: Expanding committee. Reasons: {sufficiency['reasons']}"
                        )
                        try:
                            ...
                            scaling_decision = scaling_decision_from_sufficiency(
                                sufficiency
                            )
                            ...
                            working_context["pod_orchestration"] = (
                                orchestration_state.to_dict()
                            )
                            ...
                            step_result["output"] = output
                            logger.info(
                                "Persona PodOrchestrator executed and synthesized expanded committee."
                            )
                        except Exception as e:
                            logger.error(
                                f"Persona Pod scaling failed: {e}. Reverting to base Quad."
                            )
                    ...
                    if tier in ["high_stakes", "extreme", "autonomous"]:
                        ...
                        try:
                            if self.refinement_orchestrator is None:
                                from backend.truth_engine.truth_core.refinement_orchestrator import (
                                    RefinementOrchestrator as LegacyRefinementOrchestrator,
                                )

                                self.refinement_orchestrator = (
                                    LegacyRefinementOrchestrator(
                                        ka_controller=self.ka_controller
                                    )
                                )
                            ...
                            step_result["confidence"] = refined_result[
                                "final_confidence"
                            ]
                            working_context["refinement_history"] = refined_result[
                                "refinement_history"
                            ]
                        except Exception as e:
                            logger.error(
                                f"Refinement Orchestrator crashed: {e}. Falling back to unpolished output."
                            )
            else:
        ...
        last_success = next(
            (s for s in reversed(executed_steps) if s["status"] == "completed"), None
        )
        final_answer = ""
        if last_success:
        ...
        if "persona_results" in working_context:
            logger.info(
                f"Final Persona Results Keys: {working_context['persona_results'].keys()}"
            )
        ...
        return {
            ...
            "confidence": (
                executed_steps[-1].get("confidence", 0.0) if executed_steps else 0.0
            ),
    ...
    def _execute_refinement_step(
        self, step: str, query: str, context: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        if step == "quant_validation" and self.quant_service:
            try:
                ...
                result = self.quant_service.validate(
                    draft, claims, context.get("data_context")
                )
                return {
                    "step": step,
