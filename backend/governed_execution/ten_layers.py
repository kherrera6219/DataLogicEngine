...
class GovernedTenLayerStages:
    ...
    async def l8(self, context: GovernedContext) -> LayerExecution:
        ...
        # More-secure path: fail-closed model screening + OPA after KA plan.
        screening = evaluate_model_screening(
            candidate,
            metadata={
                "request_id": context.request.request_id,
                "trace_id": context.trace_id,
        ...
        screening_block = screening.get("action") == "block" or screening.get(
            "allowed"
        ) is False

        measured_confidence = (
            float(context.confidence_measurement.value)
            if context.confidence_measurement is not None
            and context.confidence_measurement.value is not None
            else confidence
        )
        risk_domain = self._risk_domain(context)
        min_confidence = risk_domain_threshold(risk_domain)
        opa = evaluate_opa_policy(
            risk_domain=risk_domain,
            overall_confidence=measured_confidence,
            minimum_confidence=min_confidence,
            axis_17_requires_human=bool(
                context.request.metadata.get("axis_17_requires_human")
            ),
            human_reviewed=bool(context.request.metadata.get("human_reviewed")),
            simulation_id=context.trace_id,
        )
        opa_block = opa.get("allow") is not True

        ok = (
            not blocking
            and not policy_blocks
            and ka_ok
            and not screening_block
            and not opa_block
        )
        decision = GovernedPolicyDecision(
            ...
            rationale=(
                "canonical_truthgate_ka_plan_and_security_controls_committed"
                if ok
                else "validator_policy_truthgate_ka_screening_or_opa_block"
            ),
            stage="L8",
            flags=sorted(
                {
                    ...
                    *(["model_screening_block"] if screening_block else []),
                    *(["opa_policy_block"] if opa_block else []),
                }
            ),
            ka_results=ka_results,
        )
