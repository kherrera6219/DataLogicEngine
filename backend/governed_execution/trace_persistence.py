...
def _ka_output(results: dict[str, Any], ka_id: str) -> dict[str, Any]:
    payload = _mapping(results.get(ka_id))
    return _mapping(payload.get("output") or payload)


def extract_release_governance(sdk_result: dict[str, Any]) -> dict[str, Any]:
    """Collect quarantine/containment markers already present on the result."""

    metadata = _mapping(sdk_result.get("metadata"))
    lifecycle = _mapping(metadata.get("knowledge_lifecycle"))
    release = _mapping(lifecycle.get("release"))
    results = _mapping(release.get("results"))
    ka_1109 = _ka_output(results, "KA-1109")
    ka_1094 = _ka_output(results, "KA-1094")
    containment_values = [
        str(item.get("containment_class"))
        for item in _objects(ka_1109.get("decisions"))
        if item.get("containment_class")
    ]
    containment_class = None
    if "never_persist" in containment_values:
        containment_class = "never_persist"
    elif "restricted" in containment_values:
        containment_class = "restricted"
    elif containment_values:
        containment_class = containment_values[0]
    if sdk_result.get("containment_class"):
        containment_class = str(sdk_result.get("containment_class"))
    elif metadata.get("containment_class"):
    ...
    if str(memory.get("state") or "").lower() in {"quarantined", "never_persist"}:
        quarantine = True
    if sdk_result.get("quarantine") or sdk_result.get("quarantined"):
        quarantine = True
    if metadata.get("quarantine") or metadata.get("quarantined"):
        quarantine = True

    regulatory_pass = sdk_result.get("regulatory_pass")
    if regulatory_pass is None:
        regulatory_pass = metadata.get("regulatory_pass")
    security_pass = sdk_result.get("security_pass")
    if security_pass is None:
        security_pass = metadata.get("security_pass")
    if security_pass is None and (quarantine or containment_class == "never_persist"):
        security_pass = False

    return {
        "containment_class": containment_class,
        "quarantine": bool(quarantine),
        "regulatory_pass": regulatory_pass,
...
def persist_governed_trace(
    ...
    *,
    ...
    try:
        ...
        governance = extract_release_governance(sdk_result)
        if governance.get("regulatory_pass") is not None:
            run.regulatory_pass = bool(governance["regulatory_pass"])
        if governance.get("security_pass") is not None:
            run.security_pass = bool(governance["security_pass"])
