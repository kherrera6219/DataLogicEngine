...
class DataRetentionService:
    ...
    def set_policy(
        ...
        logger.info(
            f"Updated retention policy: {category.value} = {retention_days} days"
        )
        return policy
    ...
    def cleanup_trace_runs(self, dry_run: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            run_ids = [
                str(row[0]) for row in query.with_entities(TraceRun.run_id).all()
            ]
            count = len(run_ids)
            result["eligible_count"] = count

            if not dry_run and count > 0:
                purge_staged_capture_runs(run_ids)
                query.delete(synchronize_session=False)
                db.session.commit()
                result["deleted_count"] = count
    ...
    def run_cleanup(self, dry_run: bool = False) -> Dict[str, Any]:
        ...
        logger.info(
            f"Retention cleanup completed: {len(results['cleanups'])} categories processed"
        )
        return results
